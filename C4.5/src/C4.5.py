...
def binaryzation_features(trainset):
    ...
    for img in trainset:
        ...
        cv_img = img.astype(np.uint8) #转换数组的数据类型
...
class Tree(object):
    ...
    def predict(self,features):
        ...
        tree = self.dict.get(features[self.feature]) #获得该特征对应的子树
...
def recurse_train(train_set, train_label, features):
    ...
    label_set = set(train_label) #set() 函数创建一个无序不重复元素集，可进行关系测试，删除重复数据，还可以计算交集、差集、并集等。
    if len(label_set) == 1:
    ...
    for feature in features:
        ...
        if calc_ent(A) != 0:  ####### 计算信息增益比，这是与ID3算法唯一的不同
            gda /= calc_ent(A) # 计算信息增益比= 信息增益/条件熵
        if gda > max_gda:
...
if __name__ == '__main__':
    ...
    imgs = data[::,1::] #data数据的每一行除了第一列的剩余部分
    features = binaryzation_features(imgs) #图片二值化，提高预测准确率
    labels = data[::,0] #data数据的每一行的第一列

    #避免过拟合，采用交叉验证，随即选取33%数据作为测试集，剩余为训练集
    train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size = 0.33, random_state = 0) #sklearn自带函数将输入矩阵随机划分为训练集合和测试集合
    time_2 = time.time()
    print('read data cost %f seconds' %(time_2 - time_1))

    #通过C4.5算法生成决策树
    print("Start training ...")
    tree = train(train_features, train_labels, list(range(feature_len))) #range() 函数可创建一个整数列表;list()函数可以将其他数据类型转换为列表类型，并返回转换后的列表。
    time_3 = time.time()
    ...
    score = accuracy_score(test_labels, test_predict) #sklearn的函数accuracy_score(true,predict)，返回正确分类的比例
